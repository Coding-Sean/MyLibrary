...
class StaffModel extends BaseModel {
    ...
    public function processClearance($user_id, $semester) {
        ...
        if ($borrowed['count'] > 0) {
            return ['success' => false, 'message' => 'User still has unreturned books'];
        }
        ...
        if ($penalties['total'] > 0) {
            return ['success' => false, 'message' => 'User has unpaid penalties of ₱' . number_format($penalties['total'], 2)];
        }

        // Create clearance
        $query = "INSERT INTO Clearance (semester, clearanceStatus, date, user_id)
                  VALUES (:semester, 'Cleared', CURDATE(), :user_id)";
        
        $result = $this->executeQuery($query, [':semester' => $semester, ':user_id' => $user_id]);

        return $result !== false
            ? ['success' => true, 'message' => 'User cleared successfully for ' . $semester]
            : ['success' => false, 'message' => 'Failed to process clearance'];
    }
